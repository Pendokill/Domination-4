...
if (_pa isNotEqualTo []) then {
	...
	{
		if (alive _x && {_x getEntityInfo 0 && {side group _x == _sideHostile }}) then { // always use side group _x
	...
	((_pa # 0) nearEntities _distancePlayerSideTooClose) findIf {
		private _ret = alive _x && {_x getEntityInfo 0 && {side group _x == _sideHostile }};
		if (_ret) then {
			_isOccupiedByEnemy = true;
		};
		_ret
