...
if (d_tt_ver || {d_own_side == "EAST"}) then {
	...
	if (isServer) then {
		d_hq_logic_opfor2 setGroupIdGlobal ["ШТАБ"];
	};
	call {
		if (d_rhs) exitWith {
			d_hq_logic_opfor2 setVariable ["d_kddata", [["HQ_E"], "RDHQ_OP2", "ШТАБ"]];
		};
		if (d_gmcwg) exitWith {
			d_hq_logic_opfor2 setVariable ["d_kddata", [["HQ_E"], "GMDHQ_OP2", "ШТАБ"]];
		};
		d_hq_logic_opfor2 setVariable ["d_kddata", [["HQ_E"], "DHQ_OP2", "ШТАБ"]];
	};
};
...
if (d_own_side == "GUER") then {
	...
	if (isServer) then {
		d_hq_logic_guer2 setGroupIdGlobal ["ШТАБ"];
	};
	d_hq_logic_guer2 setVariable ["d_kddata", [["HQ_I"], "DHQ_IN2", "ШТАБ"]];
};
