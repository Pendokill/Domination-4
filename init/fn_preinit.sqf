...
d_x_drop_array =
	...
	call {
		if (d_rhs) exitWith {
			[[], [localize "STR_DOM_MISSIONSTRING_22", "ARHANR22_UCG_cop"], [localize "STR_DOM_MISSIONSTRING_20", "Box_East_Ammo_F"]]
		};
		if (d_ifa3) exitWith {
...
call {
	if (d_pracs) exitWith {
		...
		d_flag_str_blufor = "pics\flag_nato.paa";
	};
	if (d_spe) exitWith {
		d_flag_str_blufor = "pics\flag_nato.paa";
		d_flag_str_opfor = "\RHSAFRF\addons\rhs_main\data\flag_rus_co.paa";
...
#include "d_sm_classes_jsdf.sqf"
	...
	call {
		...
		if (d_enemy_side_short == "W") exitWith {
			d_air_radar = "I_E_Radar_System_01_F";
			d_air_radar2 = "Land_MobileRadar_01_radar_F";
			d_enemy_hq = "B_Truck_01_covered_F";
		};
		d_air_radar = "I_E_Radar_System_01_F";
		d_air_radar2 = "Land_MobileRadar_01_radar_F";
		d_enemy_hq = "B_Truck_01_covered_F";
	...
	d_airai_attack_uav = call {
		...
		if (d_enemy_side_short == "W") exitWith {
			call {
				...
				if (d_rhs) exitWith {
					["B_UGV_01_rcws_F","B_UAV_02_dynamicLoadout_F","B_T_UAV_03_dynamicLoadout_F"]
				};
				if (d_gmcwg) exitWith {
					[]
				};
	...
	d_transport_chopper = call {
		...
		if (d_enemy_side_short == "W") exitWith {
			["FP_Ger_AW101","RHS_C130J","B_T_VTOL_01_armed_F","FP_UAF_UH60_Unarmed"]
		};
		["I_Heli_Transport_02_F"]
	};
#endif
	...
	d_light_attack_chopper = call {
		...
		if (d_enemy_side_short == "W") exitWith {
			call {
				...
				if (d_rhs) exitWith {
					["RHS_UH1Y_d","RHS_MELB_AH6M_L","FP_PMC_OG_B_Huey_Gunship"]
				};
	...
	// то же, что и при строительстве казарм. Но вражеская техника с AI появляется не внутри основной зоны поражения, 
        // а снаружи, если она уничтожена, вражеская техника больше не появляется
	d_vehicle_building =
#ifdef __ALTIS__
		"Land_Cargo_HQ_V1_F";
#endif
...
if (hasInterface) then {
	...
	call {
		...
		if (d_rhs) exitWith {
			["O_G_Quadbike_01_F", "FP_Spetsnaz_Vympel_UAZ_Pickup_AGS30", "rhs_tigr_sts_3camo_vmf", "min_rf_gaz_2330_HMG"]
		};
		if (d_ifa3) exitWith {
	...
	call {
		...
		["O_MRAP_02_F", "O_MRAP_02_hmg_F", "O_Heli_Light_02_unarmed_F", "FP_Spetsnaz_Alpha_Punisher", "B_APC_Tracked_01_CRV_F", "rhsgref_BRDM2UM_vdv", "rhs_prp3_vmf", "rhs_btr80_vdv", "rhs_btr80a_vdv", "RHS_Mi8mt_vvsc", "RHS_Mi8mtv3_Cargo_vdv", "RHS_Mi8mtv3_Cargo_vvs", "rhs_ka60_c", "rhs_ka60_grey", "RHS_Mi8AMT_vvs", "RHS_Mi8mt_vdv", "RHS_Mi8amt_civilian"]
	};
#endif
#ifdef __OWN_SIDE_INDEPENDENT__
	call {
...
#endif
#ifdef __TT__
	["B_Heli_Light_01_F", "B_APC_Tracked_01_CRV_F", "O_Heli_Light_02_unarmed_F", "O_MRAP_02_F", "B_MRAP_01_F", "rhs_prp3_vmf", "rhs_btr80_vdv", "rhs_btr80a_vdv", "B_APC_Tracked_01_rcws_F", "rhsusf_m113_usarmy_MK19", "B_T_APC_Tracked_01_CRV_F", "O_MRAP_02_F", "B_MRAP_01_F", "B_APC_Tracked_01_CRV_F", "B_T_APC_Tracked_01_CRV_F", "FP_Spetsnaz_Alpha_Punisher"];
#endif
	...
	// pre build in are:
	// "CUP_"
	// "rhs_", "min_rf_", "rhsafrf_", "rhsgref_", "rhsusf_", "A3_", "rhssaf_", "VSM_", "tsp_", "real_", "Cl_", "bma3_", "F_", "jam_", "FP_", "KA_", "CWR_", "JAS_", "lago_"
	// "uns_"
	// "ace_"
	// "gm_"
	// если вы используете CUP, то в виртуальном арсенале будут показаны только предметы из CUP
	d_arsenal_mod_prestrings = ["min_rf_", "VSM_", "tsp_", "Cl_", "bma3_", "F_", "jam_", "FP_", "KA_", "CWR_", "JAS_"];

	// same as above but will remove specific mods
	// for example: "gm_" will remove Global Mobilization
	d_arsenal_mod_remove_strings = ["rhsusf_", "rhsgref_", "rhssaf_", "cup_", "CUP_", "ACE_", "ace_", "uk3cb_factions_", "lago_"];

	// can either be a class name (string) or code
	// if code then _this is the classname
	d_remove_from_arsenal = [
		[], // PRIMARYWEAPON
		["launch_min_rf_titan_short","launch_MRAWS_green_rail_F","launch_MRAWS_green_F","launch_MRAWS_olive_rail_F","launch_MRAWS_olive_F","launch_MRAWS_sand_rail_F","launch_MRAWS_sand_F","launch_MRAWS_black_F","launch_MRAWS_black_rail_F","launch_MRAWS_coyote_rail_F","rhs_weap_maaws","rhs_weap_fgm148","launch_O_Vorona_green_F","launch_O_Vorona_brown_F"], // SECONDARYWEAPON
		[], // HANDGUN
	...
	d_add_to_arsenal = [
		...
		[], // uniforms
		[], // VEST
		["O_UAV_01_backpack_F"], // BACKPACK
		[], // HEADGEAR
		["Land_DustMask_F","G_Respirator_base_F"], // GOGGLES
		[], // NVGS
		[], // BINOCULARS
		[], // MAP
		[], // GPS, UAV Terminal
		...
		[], // WATCH
		[], // FACE
		["RHS_Male01RUS","RHS_Male02RUS","RHS_Male03RUS","RHS_Male04RUS","RHS_Male05RUS","Male01RUS","Male02RUS","Male03RUS"], // VOICE
		[], // INSIGNIA
		[], // ITEMOPTIC
		[], // ITEMACC
		[], // ITEMMUZZLE
...
// поддержка переопределения врагов с помощью настроек сервера (вражеские фракции)
...
// поддержка переопределения партизанских действий с помощью настроек сервера (партизанские группировки)
...
// поддержка реакций на смерть модов и попадание в цель, это не должно быть обязательным, 
// но что-то было не так с инициализацией свойств CBA для DAH с помощью Domination, это исправлено
