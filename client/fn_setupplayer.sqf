...
#ifndef __TT__
	d_FLAG_BASE addAction [format ["<t color='#00bfff' size='1.5'>%1</t>", localize "STR_DOM_MISSIONSTRING_533"], {call d_fnc_teleportx}, -1, 1.5, true, true, "", "true", 10];
#else
	private _base_flag = [d_EFLAG_BASE, d_WFLAG_BASE] select (d_player_side == blufor);
	_base_flag addAction [format ["<t color='#00bfff' size='1.5'>%1</t>", localize "STR_DOM_MISSIONSTRING_533"], {call d_fnc_teleportx}, -1, 1.5, true, true, "", "true", 10];
#endif
};
...
if (d_arsenal_mod == 0) then {
	...
	if (d_rhs) then {
		d_arsenal_mod_prestrings append ["rhs_", "rhsgref_", "rhsafrf_", "rhssaf_"];
	};
	if (d_gmcwg) then {
