		...
		if (_x # 1 isEqualType "") then {
			if (markerPos (_x # 1) isNotEqualTo [0,0,0]) then {
				...
				if (d_player_side != _x # 3) then {
					...
					_dadao addAction [format ["<t color='#00bfff' size='1.5'>%1</t>", localize "STR_DOM_MISSIONSTRING_533"], {call d_fnc_teleportx}, -1, 1.5, true, true, "", "true", 10];
				...
				if (isNil {_dadao getVariable "d_teleidx"}) then {
					_dadao setVariable ["d_teleidx", _dadao addAction [format ["<t color='#00bfff' size='1.5'>%1</t>", localize "STR_DOM_MISSIONSTRING_533"], {call d_fnc_teleportx}, -1, 1.5, true, true, "", "true", 10]];
				};
				if (d_ParaAtBase == 0) then {
