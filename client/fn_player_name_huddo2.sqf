...
if (!d_show_pname_hud && {!visibleMap && {isNil "d_is_sat_on"}}) then {
	...
	if (alive _ct && {_ct getEntityInfo 0 && {!(player getVariable "xr_pluncon") && {_ct != player && {(positionCameraToWorld [0,0,0]) distance _ct <= (d_dist_pname_hud / 2) && {side (group _ct) getFriend d_player_side >= 0.6}}}}}) then {
