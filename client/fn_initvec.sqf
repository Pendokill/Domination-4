...
if (_d_vec < 100) exitWith {
	...
	if (_car isNotEqualTo []) then {
		...
		__vecmarker;
		_vec setVariable ["d_mhq_txt", "КШМ " +  (_car # 5)];
#ifndef __TT__
		d_marker_vecs pushBack _vec;
		_vec setVariable ["d_ism_vec", true];
	...
	if (d_player_side == blufor) then {
#endif
	_vec addAction [format ["<t color='#00bfff'>%1</t>", localize "STR_DOM_MISSIONSTRING_262"], {call d_fnc_vecdialog}, _d_vec, -1, false, true, "", "alive _target", 7];
	if (d_with_bis_dynamicgroups == 0) then {
		_vec addAction [format ["<t color='#c0c0c0'>%1</t>", localize "STR_DOM_MISSIONSTRING_552"], {call d_fnc_showdynamicgroupsdialog}, _d_vec, -1, false, true, "", "alive _target", 7];
	};
	_vec addAction [format ["<t color='#c0c0c0'>%1</t>", localize "STR_DOM_MISSIONSTRING_304"], {0 call d_fnc_DomCommandingMenuExec}, _d_vec, -1, false, true, "", "alive _target", 7];
	player reveal _vec;
#ifdef __TT__
	} else {
