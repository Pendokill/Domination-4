			...
			if (alive(uiNamespace getVariable "cti_dialog_ui_purchasemenu_factory")) then {
				...
				if (_ai_enabled == 1 || (isPlayer leader _selected_group && _ai_enabled == 0)) then {
					if ((count units _selected_group)+1 <= CTI_PLAYERS_GROUPSIZE || _isEmpty) then { //todo ai != player limit
						_proc_purchase = true;
						if (_isEmpty && _selected_group != group player) then { _proc_purchase = false; hint parseText "<t size='1.3' color='#2394ef'>Информация</t><br /><br />Empty vehicles may not be purchased for other groups."; };
						...
						if (_proc_purchase) then {
							...
							hint parseText format ["<t size='1.3' color='#2394ef'>Информация</t><br /><br />%2<t>A <t color='#ccffaf'>%1</t> is being built</t>", _get select CTI_UNIT_LABEL, _picture];
						...
						hint parseText "<t size='1.3' color='#2394ef'>Информация</t><br /><br />Your unit limit has been reached.";
					};
				} else {
					hint parseText "<t size='1.3' color='#2394ef'>Информация</t><br /><br />Units may not be purchased to AI groups while the AI Teams are disabled in the parameters.";
				};
			};
		} else {
			hint parseText "<t size='1.3' color='#2394ef'>Информация</t><br /><br />You do not have enough funds to perform this operations.";
		};
	};
			...
			if (_funds >= CTI_VEHICLES_SALVAGER_PRICE) then {
				if (count(CTI_P_SideLogic getVariable "cti_salvagers") < CTI_VEHICLES_SALVAGE_INDEPENDENT_MAX) then { 
					if (time - CTI_P_LastIndepSalvagerPurchased > 5) then {
						...
						hint parseText "<t size='1.3' color='#2394ef'>Информация</t><br /><br />Please wait a few seconds before performing this operation again.";
					};
				} else {
					hint parseText "<t size='1.3' color='#2394ef'>Информация</t><br /><br />The Independent salvager limit has been reached.";
				};
			} else {
				hint parseText "<t size='1.3' color='#2394ef'>Информация</t><br /><br />You do not have enough funds to perform this operations.";
			};
		} else {
			hint parseText "<t size='1.3' color='#2394ef'>Информация</t><br /><br />The factory shouldn't be in use in order to buy an independent salvager.";
		};
	};
	case "onQueueCancel": {
		_selected = _this select 1;
