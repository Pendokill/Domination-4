...
call {
	if (_this # 0 == "a") exitWith {
		...
		if ((_ar # 0) isKindOf d_mash) then {
			[_ar # 1, _ar # 0, "ICON", "ColorRed", [0.5, 0.5], format ["Госпиталь %1", _ar # 2], 0, "mil_dot"] call d_fnc_CreateMarkerGlobal;
#ifdef __TT__
			(_ar # 1) remoteExecCall ["deleteMarkerLocal", [blufor, opfor] select ((_ar # 4) == opfor)];
#endif
		} else {
			if ((_ar # 0) isKindOf (d_farp_classes # 0)) then {
				[_ar # 1, _ar # 0, "ICON", "ColorRed", [0.5, 0.5], format ["ПРП %1", _ar # 2], 0, "mil_dot"] call d_fnc_CreateMarkerGlobal;
#ifdef __TT__
				(_ar # 1) remoteExecCall ["deleteMarkerLocal", [blufor, opfor] select ((_ar # 4) == opfor)];
#endif
			};
