...
if (_type == 1) exitWith {
	[
		...
		/* 4 condition to show */			"!d_sm_leak_sealed && {d_player_canu && {player distance2D _target <= 8}}",
		/* 5 condition for action */		"true",
...
if (_type == 2) exitWith {
	private _idx = [
		...
		/* 2 idle icon */					"\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_hack_ca.paa",
		/* 3 progress icon */				"\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_hack_ca.paa",
		/* 4 condition to show */			"!d_sm_farpseized && {d_player_canu && {player distance2D _target <= 6}}",
		/* 5 condition for action */		"true",
